...
<b> ! На всякий случай, еще раз пошаговый алгоритм работы по веткам:
...
### На этом проекте верстаем через препроцессоры
 * Открывайте урок 7 недели и идите прямо по нему:
  * Установите node.js
  * Проверка версии установленного node.js: откройте vscode и в консоли напишите `node -v` --> если все ок - консоль выведет версию что-то типа 20.16.0
  * Установите сам sass `npm install -g sass` (у меня на этом этапе выпала ошибка что-то типа "отключено выполнение сценариев в этой системе" - погуглите, если будет такое; решается все через терминал от имени администратора введением како1-то команды).
  * Затем работаете в файлах
  * Чтобы запустить стили - их нужно скомпилировать из scss (давайте использовать именно этот синтаксис, т.к. он больше похож на css, чем sass) в css: для этого вводим команду `sass --watch [путь к исходному файлу] [путь к выходному файлу]`
пример: sass --watch assets/sass/import.scss assets/styles/style.css (первая часть - это ваша часть scss - которая, по сути, находится в import.scss; вторая часть - часть css - это общий файл стилей style.css (туда будут преобразовываться все файлы scss, поэтому не меняем style.css! если поменяете - тот образуется новый файл, куда скомпилируется ваш scss)
